...
export class NumbersOnly {
  ...
  @Input() allowIntervals: boolean = false;
  ...
  @HostListener('keydown', ['$event']) onKeyDown(e: KeyboardEvent) {
    ...
    const element = this.hostElement.nativeElement as HTMLInputElement;
    let cursorPosition: number | null = element['selectionStart'];
    let originalValue: string = element['value'];
    ...
    let allowedKeys = [
      'Delete', 'Backspace', 'ArrowLeft', 'ArrowRight', 'Escape', 'Tab'
    ];

    // when decimals are allowed, add
    // decimal separator to allowed codes when
    // its position is not close to the the sign (-. and .-)
    let separatorIsCloseToSign = (signExists && cursorPosition && cursorPosition <= 1);
    ...
    if (this.allowIntervals && !signExists) {
      allowedKeys.push('-');
    }
    ...
    this.previousValue = Number(originalValue);

    if (separatorExists) {
      let arr = originalValue.split(this.decimalSeparator);
      if (arr.length === 2) {
        if (arr[1].length > this.decimals - 1 && cursorPosition && cursorPosition > arr[0].length) {
          e.preventDefault();
        }
      }
    }
    ...
    if (this.maxlength) {
      if (originalValue.length >= this.maxlength) {
        e.preventDefault();
      }
    }
